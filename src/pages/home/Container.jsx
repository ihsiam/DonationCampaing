...
function Container(props) {
  ...
  const serachItem = props.search;

  const filteredData = Data.filter((item) =>
    item.category.toLowerCase().includes(serachItem.toLowerCase())
  );
            ...
            <Link to={title} state={data} key={id}>
              <div className='flex flex-col rounded-lg h-56 md:h-auto' style={{backgroundColor : cdBg}}>
                ...
                <div className='flex flex-col px-2 py-4'>
                  <p className='w-fit text-sm font-semibold px-2 py-1 md:px-3 md:py-2 rounded' style={{backgroundColor : ctBg, color: tbBg}}>{category}</p>
